...
namespace Wave.Native
{
	...
	public class WVR_EditorImpl : MonoBehaviour
	{
			...
			switch (type)
			{
				case WVR_DeviceType.WVR_DeviceType_Controller_Right:
					...
					if (IsButtonAvailable(type, WVR_InputId.WVR_InputId_Alias1_A))
					{
						...
						{
							DEBUG("ButtonUnpressed() " + type + ", A(X).");
							mEvent.common.type = WVR_EventType.WVR_EventType_ButtonUnpressed;
							mEvent.device.type = type;
							mEvent.input.inputId = WVR_InputId.WVR_InputId_Alias1_A;
							hasEvent = true;
					...
					if (IsButtonAvailable(type, WVR_InputId.WVR_InputId_Alias1_B))
					{
						...
						{
							DEBUG("ButtonUnpressed() " + type + ", B(Y).");
							mEvent.common.type = WVR_EventType.WVR_EventType_ButtonUnpressed;
							mEvent.device.type = type;
							mEvent.input.inputId = WVR_InputId.WVR_InputId_Alias1_B;
							hasEvent = true;
		...
		{
			if (Input.GetKeyUp(KeyCode.H))   // Down To Up Swipe
			{
			...
			if (Input.GetKeyUp(KeyCode.J))   // Right To Left Swipe
			{
			...
			if (Input.GetKeyUp(KeyCode.K))   // Up To Down Swipe
			{
			...
			if (Input.GetKeyUp(KeyCode.L))   // Left To Right Swipe
			{
		...
		{
			data.combined.eyeTrackingValidBitMask = (int)WVR_EyeTrackingStatus.WVR_GazeDirectionNormalizedValid;
