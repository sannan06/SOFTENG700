...
public class DynamicFR : MonoBehaviour
{
	...
	public WaveXR_FoveatedRendering foveated;

	public bool useResolutionScale = false;
	...
	{
		return Mathf.Clamp(Mathf.FloorToInt((FOV_Wide - FOV_Narrow) * (1 - motionScale * FRWeight) + FOV_Narrow), 38, 75);
	}
	...
	void Update()
	{
		if (Input.GetButtonUp("Button6") || Input.GetButtonUp("Button7"))
			applyDynamicUpdate = !applyDynamicUpdate;
	...
	private void OnValidate()
	{
		if (FOV_Wide <= FOV_Narrow)
		{
			...
			FOV_Wide = Mathf.Clamp(FOV_Wide, 38, 75);
			if (FOV_Wide == FOV_Narrow)
				FOV_Narrow -= 1;
		}
	}
