...
{
	public class WaveXR_FoveatedRendering : MonoBehaviour
	{
		public static string TAG = "WXR_Foveated";
		public static string StaticTrackedObjectName = "StaticFoveatedTrackedObject";

		public static WaveXR_FoveatedRendering Instance { get; private set; }
		...
		void CreateTrackedObject()
		{
			Transform t;
			if (staticTrackedObject != null)
				t = staticTrackedObject.transform;
			else
				t = Camera.main.transform.Find(StaticTrackedObjectName);
			...
			if (t == null)
			{
				...
				t.SetParent(Camera.main.transform, false);
			}
		...
		WVR_FoveationMode originalMode = WVR_FoveationMode.Default;
		void OnEnable()
		{
			if (!isEnabled)
			{
		...
		void OnDisable()
		{
			if (isEnabled)
			{
				...
				if (!Application.isEditor)
#endif
					Interop.WVR_RenderFoveationMode(originalMode);
				isEnabled = false;
			}
		}
		...
		void doUpdate()
		{
			...
			// Left eye
			LeftNDCSpace = WorldToNDC_GL(Camera.main, Camera.StereoscopicEye.Left, worldfocalPoint);
			...
			// Right eye
			RightNDCSpace = WorldToNDC_GL(Camera.main, Camera.StereoscopicEye.Right, worldfocalPoint);
		...
		public static Vector2 WorldToNDC_GL(Camera camera, Camera.StereoscopicEye eye, Vector3 worldPoint)
		{
			Matrix4x4 mat = camera.projectionMatrix * camera.GetStereoViewMatrix(eye);
		...
		private static void SetFoveatedRenderingParameterCheck(WVR_Eye eye, float ndcFocalPointX, float ndcFocalPointY, float clearVisionFOV, WVR_PeripheralQuality quality)
		{
            ...
            Functions.SetFoveatedRenderingParameter?.Invoke((uint)eye, clearVisionFOV, (uint)quality, ndcFocalPointX, ndcFocalPointY);
        }
		...
		{
            WaveXR_FoveatedRendering.useValuesInInspector = true;
