...
namespace Wave.Essence.Editor
{
	public static class DummyPose
	{
		...
		private static WVR_DevicePosePair_t[] wvrPoses = new WVR_DevicePosePair_t[WaveEditor.DeviceCount];
		private static RigidTransform[] rtPoses = new RigidTransform[WaveEditor.DeviceCount];
		...
		public static IEnumerator UpdatePoses()
		{
			while (ToUpdatePose)
			{
				...
				Interop.WVR_GetSyncPose(Origin, wvrPoses, (uint)WaveEditor.DeviceCount);
				for (uint wvr_index = 0; wvr_index < WaveEditor.DeviceCount; wvr_index++)
				{
					WVR_DevicePosePair_t wvr_pose = wvrPoses[wvr_index];
					for (uint rt_index = 0; rt_index < WaveEditor.DeviceCount; rt_index++)
					{
