...
class AndroidPlatformCheck {
  ...
  static void Check() {
    ...
    if (hasGooglevrPlugin && hasWavevrPlugin) {
      bool showDialog =
          EditorPrefs.GetBool("ViveHandTracking.AndroidPlatformCheck.ShowDialog", true);
      if (showDialog) {
        bool result = EditorUtility.DisplayDialog(
            ...
            "Both plugins cannot work together and Vive Hand Tracking plugin cannot determine which API to use," +
                "Please add " + GoogleVRDefine + " or " + WaveVRDefine +
                " to android scripting define symbols manually.",
            "Got it", "Skip Checks");
        if (!result) File.WriteAllText(IgnoreFilePath, "");
      } else
        Debug.LogWarningFormat(
            "Vive Hand Tracking detected both GoogleVR and WaveVR plugin, please add {0} or {1} to android scripting define symbols manually.",
