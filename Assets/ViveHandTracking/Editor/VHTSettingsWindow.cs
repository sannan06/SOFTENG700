...
class VHTSettingsWindow : EditorWindow {
  static VHTSettingsWindow() { EditorApplication.update += Update; }

  static void Update() {
    var settings = VHTSettings.GetSettings(false);
    if (settings == null) ShowWindow();
  ...
  void OnGUI() {
    if (settings == null) settings = VHTSettings.GetSettings(true);
    if (drawer == null) drawer = new VHTSettingsDrawer(new SerializedObject(settings));
